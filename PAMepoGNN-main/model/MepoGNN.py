...
class stcell(nn.Module):
    def __init__(self, num_nodes, dropout, in_dim, out_len, residual_channels, dilation_channels, skip_channels,
        ...
        self.stable_chara = torch.nn.Parameter(torch.randn([1, 32, 47, 1]))
        self.mapping = torch.nn.Parameter(torch.randn([4, 32]))
        self.start_conv = nn.Conv2d(in_channels=32,
    ...
    def forward(self, input, adp_g):
        ...
        input += self.stable_chara.repeat(input.shape[0], 1, 1, input.shape[3])
